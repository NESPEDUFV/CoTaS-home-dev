...
NS_LOG_COMPONENT_DEFINE("smartHouse_60_average");

int main(int argc, char* argv[])
{
// ----------------- Configura logs do terminal -------------------------

    LogComponentEnable("smartHouse_60_average", LOG_LEVEL_INFO); 
...
// 16 aplicações + CoTaS
    ...
    NodeContainer celularNodes;
    celularNodes.Create(3);
    ...
    NodeContainer tomadaNodes;
    tomadaNodes.Create(10);
...
// ------------------------ Agrupa todos os nós -------------------------
    ...
    NodeContainer allNodes = NodeContainer( 
        computadorNode, espelhoNodes, televisaoNode, celularNodes,
        echoDotNodes, cameraNodes, guardaRoupaNode, portaNodes,
        armarioCozinhaNode, armarioBanheiroNode, relogioNode,
        carroNode, fogaoNode, geladeiraNode, gotejadorNode,
        colarNode, coleiraNode, comedouroNode, RAPNode, tomadaNodes,
        lavaLouçasNode, interruptorNodes, aCNodes, panelaNode,
...
// --------------------- Configurando rede wifi --------------------------
    ...
    NetDeviceContainer 
    computadorDev, espelhoDevs, televisaoDev, celularDevs,
    echoDotDevs, cameraDevs, guardaRoupaDev, portaDevs,
    armarioCozinhaDev, armarioBanheiroDev, relogioDev,
    carroDev, fogaoDev, geladeiraDev, gotejadorDev,
    colarDev, coleiraDev, comedouroDev, RAPDev, tomadaDevs,
    lavaLouçasDev, interruptorDevs, aCDevs, panelaDev,
    anelDev, cafeteiraDev, janelaCortinaDevs, lampadaDevs, 
    roteadorDevs, CoTaSDev, ApplicationsDevs;
    ...
    espelhoDevs = wifi.Install(phy, mac, espelhoNodes);
    televisaoDev = wifi.Install(phy, mac, televisaoNode);
    celularDevs = wifi.Install(phy, mac, celularNodes);
    echoDotDevs = wifi.Install(phy, mac, echoDotNodes);
    cameraDevs = wifi.Install(phy, mac, cameraNodes);
    guardaRoupaDev = wifi.Install(phy, mac, guardaRoupaNode);
    portaDevs = wifi.Install(phy, mac, portaNodes);
    ...
    tomadaDevs = wifi.Install(phy, mac, tomadaNodes);
    lavaLouçasDev = wifi.Install(phy, mac, lavaLouçasNode);
    interruptorDevs = wifi.Install(phy, mac, interruptorNodes);
    aCDevs = wifi.Install(phy, mac, aCNodes);
    ...
    janelaCortinaDevs = wifi.Install(phy, mac, janelaCortinaNodes);
    lampadaDevs = wifi.Install(phy, mac, lampadaNodes);
    
    CoTaSDev = wifi.Install(phy, mac, CoTaSNode);
    ApplicationsDevs = wifi.Install(phy, mac, ApplicationsNodes);
...
// ------------------------- Objetos inteligentes ----------------------
    ...
    position = computadorNode.Get(1)->GetObject<MobilityModel>(); // 2
    position->SetPosition(Vector(6, 14, 0.1)); // Computador 2
    ...
    position = televisaoNode.Get(1)->GetObject<MobilityModel>(); // 4
    position->SetPosition(Vector(4, 11, 0.3)); // Televisão
    ...
    position = relogioNode.Get(0)->GetObject<MobilityModel>(); //23
    position->SetPosition(Vector(3, 6, 0.0)); // relogio

    mobility.Install(relogioNode);
    position = relogioNode.Get(1)->GetObject<MobilityModel>(); //23
    ...
    mobility.Install(celularNodes);
    position = celularNodes.Get(0)->GetObject<MobilityModel>(); //70
    position->SetPosition(Vector(1, 1, 0.0)); // celular
    
    mobility.Install(celularNodes);
    position = celularNodes.Get(1)->GetObject<MobilityModel>(); //70
    position->SetPosition(Vector(1, 5, 0.0)); // celular

    mobility.Install(celularNodes);
    position = celularNodes.Get(2)->GetObject<MobilityModel>(); //70
    position->SetPosition(Vector(5, 1, 0.0)); // celular
    ...
    mobility.Install(coleiraNode);
    position = coleiraNode.Get(1)->GetObject<MobilityModel>(); //28
    position->SetPosition(Vector(5, 17, 0.0)); // Coleira
    ...
    position = panelaNode.Get(1)->GetObject<MobilityModel>(); //59
    position->SetPosition(Vector(19, 4, 2.0)); // Panela 1    

    position = portaNodes.Get(0)->GetObject<MobilityModel>(); //62
    position->SetPosition(Vector(4, 7.1, 2.1)); // Porta sala ▲ 2
    
    position = portaNodes.Get(1)->GetObject<MobilityModel>(); //63
...
// ------------------------ Atribuir endereços IP -------------------
    ...
    Ipv4InterfaceContainer espelhoInterface = ipv4.Assign(espelhoDevs);
    Ipv4InterfaceContainer televisaoInterface = ipv4.Assign(televisaoDev);
    Ipv4InterfaceContainer celularInterface = ipv4.Assign(celularDevs);
    Ipv4InterfaceContainer echoDotInterface = ipv4.Assign(echoDotDevs);
    Ipv4InterfaceContainer cameraInterface = ipv4.Assign(cameraDevs);
    Ipv4InterfaceContainer guardaRoupaInterface = ipv4.Assign(guardaRoupaDev);
    Ipv4InterfaceContainer portaInterface = ipv4.Assign(portaDevs);
    ...
    Ipv4InterfaceContainer tomadaInterface = ipv4.Assign(tomadaDevs);
    Ipv4InterfaceContainer lavaLouçasInterface = ipv4.Assign(lavaLouçasDev);
    Ipv4InterfaceContainer interruptorInterface = ipv4.Assign(interruptorDevs);
    Ipv4InterfaceContainer aCInterface = ipv4.Assign(aCDevs);
    ...
    Ipv4InterfaceContainer janelaCortinaInterface = ipv4.Assign(janelaCortinaDevs);
    Ipv4InterfaceContainer lampadaInterface = ipv4.Assign(lampadaDevs);
    ...
    Ipv4InterfaceContainer AppInterfaces = ipv4.Assign(ApplicationsDevs);
    ...
    televisaoServerApp, celularServerApps, echoDotServerApp, 
    cameraServerApp, guardaRoupaServerApp, portaServerApps,
    armarioCozinhaServerApp, armarioBanheiroServerApp, relogioServerApp,
    carroServerApp, fogaoServerApp, geladeiraServerApp, gotejadorServerApp,
    colarServerApp, coleiraServerApp, comedouroServerApp, RAPServerApp, 
    tomadaServerApps, lavaLouçasServerApp, 
    aCServerApp, panelaServerApp, anelServerApp, cafeteiraServerApp, 
    janelaCortinaServerApp, lampadaServerApp, interruptorServerApp;
    ...
    portaServerApps = ObjectService.Install(portaNodes);
    ...
    ObjectService.SetAttribute("ObjectType", UintegerValue(31));
    tomadaServerApps = ObjectService.Install(tomadaNodes);

    ObjectService.SetAttribute("ObjectType", UintegerValue(32));
    celularServerApps = ObjectService.Install(celularNodes);
    ...
    serverApplications.Add(portaServerApps);
    serverApplications.Add(relogioServerApp);
    serverApplications.Add(RAPServerApp);
    serverApplications.Add(tomadaServerApps);
    serverApplications.Add(celularServerApps);
    ...
    ApplicationContainer portaApps = ClientProvider.Install(portaNodes);
    ...
    ClientProvider.SetAttribute("ObjectType", UintegerValue(31));
    ApplicationContainer tomadaApps = ClientProvider.Install(tomadaNodes);

    ClientProvider.SetAttribute("ObjectType", UintegerValue(32));
    ApplicationContainer celularApps = ClientProvider.Install(celularNodes);
    ...
    providerApplications.Add(portaApps);
    providerApplications.Add(relogioApp);
    providerApplications.Add(RAPApp);
    providerApplications.Add(tomadaApps);
    providerApplications.Add(celularApps);
